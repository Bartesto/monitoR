...
swan_surfR <- function(path, ovit, ocav){
  suppressWarnings({
    ...
    if(length(locations) == 2){
      ...
      # idw_list_n_s <- vector("list", length(vals))
      # names(idw_list_n_s) <- vals
      ...
      #
      #   tpsmodnar <- Tps(coordinates(d1nar), d1nar$value)
      ...
      #   idw1_dfnar <- idw1_dfnar[-(1:306),] # strip weird close to -0.1 vals
      #   idw_list_n_s[[i]] <- idw1_dfnar
      # }
      #
      # make sample collection points
      samp_locs <- comb_data %>%
