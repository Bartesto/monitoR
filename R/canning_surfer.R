...
#' @examples
#' \dontrun{
#' canning_surfR(path = "Z:/DEC/MonitoringProgram/Data")
#' }
#'
  ...
  for(pair in data_pairs){
    ...
    salPlot <- ggplot()+
      ...
      geom_text(data = samp_labels_locs,
                aes(x = dist_bridg/1000, y = 0.7, label = site),
                size = 4.5,
                colour = "black",
                alpha = 1,
                check_overlap = TRUE) +
    ...
    chlorPlot <- ggplot()+
      ...
      geom_point(data = samp_locs,
                 ...
                 size = 0.5) +
      annotate("text",
               label = expression(paste("Chlorophyll (", mu,"/L)")),
               x = 5,
               y = -6.2,
               size = 7,
               colour = "black") +
    ...
    surfers <- grid.arrange(salPlot, doPlot, chlorPlot, tempPlot, nrow = 4,
                            top = textGrob(paste("Canning River Estuary - Physical-Chemical Profile -",
